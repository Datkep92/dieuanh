// script.js
// Quản lý tồn kho từ Excel — bản hoàn chỉnh với Firebase Realtime Database + Error Handling + Fix History
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderStock() {
    ...
    arr.forEach((it, idx) => {
      ...
      // Sửa ở đây: sử dụng qtyReal thay vì real, và tính toán thành tiền đúng
      ...
      tr.innerHTML = `
        <td>${idx + 1}</td>
        <td contenteditable="true" class="col-name">${esc(it.name)}</td>
        <td contenteditable="true" class="col-unit">${esc(it.unit)}</td>
        <td contenteditable="true" class="col-qty num">${soLuongTon}</td>
        <td contenteditable="true" class="col-price num">${donGia}</td>
  ...
  async function saveEditedStockFromTable() {
    ...
    rows.forEach((tr) => {
      ...
      const qtyReal = Number(tds[3].innerText) || 0; // Sửa: qtyReal thay vì real
      const price = Number(tds[4].innerText) || 0;
      const amount = qtyReal * price; // Sửa: tính toán lại amount
      
      obj[`${name}___${unit}`] = { 
        name, 
        unit, 
        qtyReal, // Sửa: qtyReal thay vì real
        price, 
        amount 
      };
    });
